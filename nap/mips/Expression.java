...
public class Expression implements ir.expr.Visitor<List<String>> {
	...
	// TODO: done?
        Register arrayRegister = exp.getRegister();
        int registerOffset = regAddress.get(arrayRegister);
        List<String> asmCode = exp.accept(this);
        asmCode.addAll(Asm.pop("$t1"));
        asmCode.add(Asm.command(Asm.load(Program.DEFAULT_SIZE) + " " + registerOffset + "($fp"));
        asmCode.add(Asm.command("lw $t2, " + registerOffset));
        asmCode.add(Asm.command("add $t2, $t2, $t0"));
        asmCode.add(Asm.command("load $t3, ($t2)"));
	...
	// TODO: Done?
        Register arrayRegister = exp.getRegister();
        int registerOffset = regAddress.get(arrayRegister);
        List<String> asmCode = exp.getOffset().accept(this);
        asmCode.addAll(Asm.pop("$t1"));
        asmCode.add(Asm.command(Asm.load(Program.DEFAULT_SIZE) + " " + registerOffset + "($fp"));
        asmCode.add(Asm.command("lw $t2, " + registerOffset));
        asmCode.add(Asm.command("add $t2, $t2, $t0"));
